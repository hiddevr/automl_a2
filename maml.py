...
class MAML(nn.Module):
    ...
    def apply(self, x_supp, y_supp, x_query, y_query, training=False):
        # use this: https://github.com/facebookresearch/higher
        ...
        # Flatten and organize initial parameters
        fast_weights = [param.clone() for _, param in self.network.named_parameters()]

        # Inner loop: Update the parameters based on the support set
        for _ in range(self.num_updates):
            preds = self.network(x_supp, weights=fast_weights)
            loss = self.inner_loss(preds, y_supp)

            # Compute gradients
            grads = torch.autograd.grad(loss, fast_weights, create_graph=self.second_order)

            # Update weightsxx
            fast_weights = [param - self.inner_lr * grad for param, grad in zip(fast_weights, grads)]

        # Compute predictions and loss on the query set using the updated parameters
